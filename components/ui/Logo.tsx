 ...
 * Die Pupillen folgen dem Mauszeiger (geteilte Logik mit GooglyEyes).
 */
export function Logo({
  ...
  // „boogly" → vorderer Teil + zwei Augen + hinterer Teil (ersetzt das „oo")
  ...
  const eye = (
    <span
      ...
      style={{ width: '0.62em', height: '0.62em', margin: '0 .015em', transform: 'translateY(.04em)', background: 'var(--paper)' }}
    >
      <i
        data-pupil
        className="absolute rounded-full"
        style={{ width: '42%', height: '42%', background: 'var(--ink)', transition: 'transform .12s ease-out' }}
      />
    </span>
  )
  ...
  return (
    <Link
      ...
      style={{ fontSize: size, letterSpacing: '-0.04em', color: 'var(--paper)' }}
    >
      {pre}
      {ooIndex >= 0 ? (
        <>
