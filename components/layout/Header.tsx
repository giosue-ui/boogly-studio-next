...
export function Header({ settings }: { settings?: SiteSettings | null }) {
  ...
  return (
    <header
      ...
      style={{ background: 'rgba(242,238,228,0.92)', borderColor: 'rgba(26,26,24,0.08)' }}
    >
      <div className="flex items-center justify-between px-6 sm:px-10 py-4">
        <Logo brand={brandName} suffix={brandSuffix} size={25} onLight />
          ...
          {nav.map((item) =>
            item.children?.length ? (
              <div key={item.label} className="relative group">
                <button className="flex items-center gap-1.5 text-[15px] font-medium text-[#1A1A18] hover:text-accent transition-colors">
                ...
                <div
                  ...
                  style={{ background: '#fcfaf4', border: '1px solid #e3dccc', boxShadow: '0 22px 50px rgba(26,26,24,.14)' }}
                >
              ...
              <Link
                key={item.label}
                href={item.href || '#'}
                className="text-[15px] font-medium text-[#1A1A18] hover:text-accent transition-colors"
              >
                {item.label}
              </Link>
            )
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden border-t px-6 py-4 space-y-1" style={{ borderColor: 'rgba(26,26,24,0.08)' }}>
          {nav.flatMap((item) =>
            item.children?.length
              ? item.children.map((c) => (
...
function DropdownLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href || '#'}
      className="block px-[15px] py-[11px] rounded-[10px] text-[14.5px] text-[#5a554c] hover:text-[#1A1A18] hover:bg-black/[0.05] transition-colors"
    >
      {label}
    </Link>
  )
...
function MobileLink({ href, label, onClick }: { href: string; label: string; onClick: () => void }) {
  return (
    <Link href={href || '#'} className="block px-2 py-2 text-[#1A1A18] hover:text-accent transition-colors" onClick={onClick}>
      {label}
    </Link>
  )
}
